...
public class WifiScanner extends Activity
{
	...
	public void testGeo(List<ScanResult> listSort)
	{
		...
		double[] geo = trilateration.MyTrilateration(51.451690, 5.481781, trilateration.calcDistance(test.get(0).level), 51.451563, 5.481635, trilateration.calcDistance(test.get(1).level), 51.451587, 5.481856, trilateration.calcDistance(test.get(2).level));
		
		Toast toast = Toast.makeText(getApplicationContext(), ""+ geo[0] + " - " + geo[1], Toast.LENGTH_SHORT);
		toast.show();
		...
		for (ScanResult s : listSort)
		{
			for (int i = 0; i < macAdresses.length; i++)
			{
				if (s.BSSID.equals(macAdresses[i]))
				{
					...
					double distance = trilateration.calcDistance(convertDbmToRSSI(s.level));
